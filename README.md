...
## Levande bakgrund (visuell finslipning)

Efterfrågat direkt av användaren — appens bakgrund var tidigare en helt platt färg (`--bg`).
`src/index.css` har nu ett `body::before`-lager: två mjukt urblurrade glöd i appens egna
accentfärger (`--accent`/`--accent-2`), på ett element större än viewporten (`-15vh`/`-15vw`
i varje riktning) som sakta translateras/skalas via en `@keyframes`-animation (24s,
`ease-in-out infinite alternate`) — de överdimensionerade kanterna gör att rörelsen aldrig
avslöjar en tom kant. `position: fixed` + `z-index: -1` + `pointer-events: none` håller det
bakom allt innehåll och stör aldrig klick/scroll. Respekterar `prefers-reduced-motion:
reduce` (fryser animationen istället för att ta bort glöden helt). Ren CSS, ingen canvas/JS —
försumbar prestandapåverkan.

**Korrigering (första försöket hade ingen synlig effekt):** animerade först `background-position`
på en radial-gradient som redan täckte hela elementet (`background-size` default är elementets
egen storlek) — utan tomrum att panorera i hade `background-position` ingen synlig effekt alls.
Bytt till att animera `transform: translate/scale` på ett överdimensionerat element istället,
en betydligt mer robust teknik som inte är beroende av `background-size`-nyanser.
