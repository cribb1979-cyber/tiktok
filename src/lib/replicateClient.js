...
async function submitBroll({ category, subtopic, hookText, customPrompt }) {
  const response = await fetch('/api/generate-broll', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ category, subtopic, hookText, customPrompt }),
  })
...
export async function generateBroll({ category, subtopic, hookText, customPrompt, onStatus }) {
  const { taskId, prompt } = await submitBroll({ category, subtopic, hookText, customPrompt })
