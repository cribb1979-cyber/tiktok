...
async function submitBroll({ category, subtopic, hookText }) {
  const response = await fetch('/api/generate-broll', {
    ...
    body: JSON.stringify({ category, subtopic, hookText }),
  })
...
export async function generateBroll({ category, subtopic, hookText, onStatus }) {
  const { taskId, prompt } = await submitBroll({ category, subtopic, hookText })
